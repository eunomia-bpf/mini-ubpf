				 ...
				 int dst_reg)
{
	...
	if (!ctx->prog->aux->extable ||
	    WARN_ON_ONCE((u32)ctx->exentry_idx >= ctx->prog->aux->num_exentries))
		return -EINVAL;
...
static int validate_code(struct jit_ctx *ctx)
{
	...
	if (WARN_ON_ONCE((u32)ctx->exentry_idx != ctx->prog->aux->num_exentries))
		return -1;
